...
namespace LabMediaWidget
{
    public partial class MainWindow : Window
    {
        ...
        private void Window_PreviewMouseWheel(object sender, MouseWheelEventArgs e)
        {
            try
            {
                if (_ytmdState.Active)
                {
                    ...
                    _panel?.SyncVolumeLevel((float)(next / 100.0));
                    e.Handled = true;
                    return;
                }
                ...
                if (state.HasSession)
                {
                    appHint = string.IsNullOrWhiteSpace(state.SourceAppId) ? state.SourceApp : state.SourceAppId;
                }
                ...
                _volToastTimer.Stop();
                _volToastTimer.Start();
                _panel?.SyncVolumeLevel(currentVol);
        ...
        private void ApplyAdaptiveLayout()
        {
            ...
            // Three transport buttons occupy 84 DIP (25 + 3 + 28 + 3 + 25).
            // Keep the rail exact so even the 140-DIP micro layout cannot clip
