...
async function testConnectedLibraryFlow() {
  ...
  assert.deepStrictEqual(state.library.playlists.map(item => item.title), ['Liked Videos', 'Owned playlist']);
  ...
  assert.deepStrictEqual(state.library.playlists.map(item => item.title), ['Liked Videos', 'Owned playlist', 'Second playlist']);
